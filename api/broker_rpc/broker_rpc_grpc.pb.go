...
func (c *brokerClient) PullMessages(ctx context.Context, in *PullMessagesRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[PullMessagesResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Broker_ServiceDesc.Streams[1], Broker_PullMessages_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Broker_PullMessagesClient = grpc.ServerStreamingClient[PullMessagesResponse]
...
func (UnimplementedBrokerServer) PullMessages(*PullMessagesRequest, grpc.ServerStreamingServer[PullMessagesResponse]) error {
...
func _Broker_PullMessages_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(PullMessagesRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(BrokerServer).PullMessages(m, &grpc.GenericServerStream[PullMessagesRequest, PullMessagesResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Broker_PullMessagesServer = grpc.ServerStreamingServer[PullMessagesResponse]
