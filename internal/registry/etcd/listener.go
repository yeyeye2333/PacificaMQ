...
type listener struct {
	common.Listener
	rootPath, partitionPrefix, consumerPrefix string
}

func newListener(l common.Listener, rootPath, partitionPrefix, consumerPrefix string) *listener {
	return &listener{
		Listener:        l,
...
func (l *listener) initialize(response *clientv3.GetResponse) {
	var parData, conData, leaderData []interface{}
...
func (l *listener) parse(k, v string) (interface{}, error) {
	...
	case strings.HasPrefix(k, l.partitionPrefix):
		parInfo := &common.PartitionInfo{
			TopicName:   strs[1],
			PartitionID: conv.BytesToInt32([]byte(strs[2])),
		}
		if v != "" {
